...
use midnight_circuits::{compact_std_lib::{self, MidnightCircuit, Relation}, ecc::{hash_to_curve::HashToCurveGadget, native::EccChip, native::ScalarVar}, field::AssignedNative, hash::poseidon::PoseidonChip, instructions::{
...
}, testing_utils::plonk_api::filecoin_srs, types::AssignedNativePoint, verifier};
...
fn test_ivc_with_inner() {
    ...
    let duration = start.elapsed(); // Measure the elapsed time after proof generation.
    println!("\ninner circuit vk pk generation took: {:?}", duration);
    ...
    let inner_proof = compact_std_lib::prove::<Certificate, PoseidonState<F>>(
        &inner_srs, &inner_pk, &relation, &inner_instance, inner_witness, OsRng,
    )
    .expect("Proof generation should not fail");
    let duration = start.elapsed(); // Measure the elapsed time after proof generation.
    println!("\nProof generation took: {:?}", duration);
    ...
    let mut fixed_bases = BTreeMap::new();
    fixed_bases.insert(String::from("com_instance"), C::identity());
    fixed_bases.extend(verifier::fixed_bases("inner_vk", &inner_vk.vk()));
  ...
  //  println!("\nfixed bases from verifier: {:?}", crate::verifier::fixed_bases::<C>("inner_vk", &inner_vk.vk()));
   ...
   // println!("inner acc = {:?}", inner_acc);
    ...
    fixed_bases.extend(verifier::fixed_bases("self_vk", &vk));
    let fixed_base_names = fixed_bases.keys().cloned().collect::<Vec<_>>();
    ...
    println!("IVC proof created in {:?}", start.elapsed());
    println!("proof size {:?}", proof.len());
