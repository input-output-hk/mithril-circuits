...
pub struct IvcCircuit {
    ...
    pub inner_merkle_root: Value<F>,
    pub inner_msg: Value<F>,
...
impl Circuit<F> for IvcCircuit {
    ...
    fn synthesize(
...

        {
            ...
            // If it is genesis, select genesis msg as msg; otherwise, select inner msg as msg.
            let msg = native_gadget.select(&mut layouter, &is_genesis, &genesis_msg, &inner_msg)?;
            native_gadget.constrain_as_public_input(&mut layouter, &msg)?;

            let zero = native_gadget.assign_fixed(&mut layouter, F::ZERO)?;
            let merkle_root =
                native_gadget.select(&mut layouter, &is_genesis, &zero, &inner_merkle_root)?;
...

            {
                ...
                let mut is_equal = native_gadget.is_equal(
                    &mut layouter,
                    &inner_merkle_root,
                    &prev_next_merkle_root,
                )?;
                is_equal = native_gadget.or(&mut layouter, &[is_equal, is_genesis.clone()])?;
                native_gadget.assert_equal_to_fixed(&mut layouter, &is_equal, true)?;
            }
...

            {
                ...
                let mut is_equal = native_gadget.is_equal(&mut layouter, &next, &current_epoch)?;
                // If it is genesis, we don't care about the previous epoch number
                is_equal = native_gadget.or(&mut layouter, &[is_equal, is_genesis])?;
                native_gadget.assert_equal_to_fixed(&mut layouter, &is_equal, true)?;
            }
        }

        {
            ...
            // [genesis_msg, genesis_vk, prev_state, prev_msg, prev_next_merkle_root, prev_current_epoch, inner_vk, self_vk, prev_acc]
            let assigned_pi = [
                vec![
                    genesis_msg.clone(),
...
mod tests {
    ...
    fn test_ivc_sd() {
        ...
        let inner_vk = zk::setup_vk(&inner_srs, &inner_relation);
        let inner_pk = zk::setup_pk(&inner_relation, &inner_vk);
        ...
        let mut inner_proofs = vec![vec![]];
        // We don't need to use inner_trivial_acc
        let mut inner_accs = vec![inner_trivial_acc];
        for i in 1..NUM_CERT {
            let start = Instant::now();
            let inner_proof = zk::prove::<Certificate, PoseidonState<F>>(
                &inner_srs,
                &inner_pk,
                &inner_relation,
        ...
        combined_fixed_bases.extend(inner_fixed_bases.clone());
        ...
        for i in 0..NUM_CERT {
            let circuit = IvcCircuit {
                ...
                inner_merkle_root: Value::known(merkle_roots[i]),
                inner_msg: Value::known(msgs[i]),
                inner_proof: Value::known(inner_proofs[i].clone()),
            ...
            let public_inputs = [
                ...
                AssignedVk::<S>::as_public_input(&inner_vk.vk()),
...

        {
            ...
            for _ in 0..total {
                ...
                let mut accumulated = Accumulator::accumulate(&[proof_acc, prev_acc.clone()]);
                accumulated.collapse();
